...
interface FunctionResponseFormProps {
  onSubmit: (responseData: string) => void;
  onCancel: () => void;
}

export function FunctionResponseForm({ onSubmit, onCancel }: FunctionResponseFormProps) {
  ...
  const handleSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const responseData = responseType === 'weather' 
      ? JSON.stringify({
          temperature: Number(weatherResponse.temperature || '19'),
          unit: weatherResponse.unit || 'C',
          conditions: weatherResponse.conditions || 'clear'
        })
      : JSON.stringify({
          function: 'clarify',
          parameters: {
            question: clarifyQuestion || 'Could you specify which city you are asking about?'
          }
        });
